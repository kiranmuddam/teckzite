...
if($money['paid']=="no" && $eve_id!=68){$err=$err.$tids[$i].", ";$regco++;}
else{$valid++;}	
}

if($regco!=0){$err=$err." havent completed payment process yet.";echo $err;exit;}
...
if(count($tids)>$query_fetch['participants']){
	...
	if(mysql_query("INSERT INTO event_registrations(`eid`, `branch`, `eventname`,`teamid`,`ids`,`regdone_by`,`regdone_ip`) VALUES ('$eid', '$branch','$eventname', '$teamid','$ids','$stuid','$ip')")){
	for($i=0;$i<count($tids);$i++)
	{
	$query=mysql_query("SELECT * FROM events WHERE eid='$eid'");
    $query_fetch=mysql_fetch_array($query);
	...
	echo "success";
		...
		else{echo "Failed in registering to events. Contact Technical Team.";}
	}
}
}
else
